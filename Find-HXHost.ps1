function Find-HXHost {
    ...
    process {
        # Uri filtering:
        if ($Uri -match '\d$') { $Endpoint = $Uri + "/hx/api/v3/hosts/?" }
        elseif ($Uri -match '\d/$') { $Endpoint = $Uri + "hx/api/v3/hosts/?" }
        else { $Endpoint = $Uri + "/?" }
        ...
        if ($HasAlerts) { $Endpoint = $Endpoint + "&has_alerts=1" }
